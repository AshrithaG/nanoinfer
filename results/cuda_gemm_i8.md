# int8 GEMM on RTX 4090 (sm_89), mean ms per launch over back-to-back launches
...
cuBLAS in this table is called with B row-major (NN), which is the call `gemm_i8.cu` makes. Given both operands K-contiguous (TN) it runs 1.8x to 3.4x faster, and with host launch cost excluded it is ahead of every kernel here at every size. See `triton_gemm_i8.md`.
